...
function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <div className={`${isOpen ? 'block' : 'hidden'} fixed inset-0 bg-black/25 z-[70]`} onClick={onClose}>
      <aside className="absolute right-0 top-0 bottom-0 w-full sm:w-[400px] lg:w-[350px] bg-white p-6 sm:p-8 lg:p-10 shadow-2xl overflow-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-6">
          <h3 className="font-bold text-3xl sm:text-3xl lg:text-2xl text-text mb-2 text-slate-700">Contact Info</h3>
          <button className="text-2xl text-slate-700 lg:text-xl leading-none" aria-label="Close" onClick={onClose}>×</button>
        </div>
        
        {/* Navigation - Only visible on mobile (below sm breakpoint) */}
        <div className="block sm:hidden border-b border-slate-200 text-slate-700 py-3 mb-6">
        ...
        <div className="border-b border-slate-200 text-slate-700 py-3 mb-6">
          <div className="flex items-center gap-4 sm:gap-6 lg:gap-4 py-4 sm:py-6 lg:py-4">
            <span className="w-6 h-6 sm:w-8 sm:h-8 lg:w-6 lg:h-6 grid place-items-center text-primary flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 sm:w-10 sm:h-10 lg:w-8 lg:h-8"><path d="M2.25 6.75c0-1.243 1.007-2.25 2.25-2.25h2.04c.995 0 1.86.67 2.108 1.636l.519 2.078a2.25 2.25 0 01-.563 2.157l-1.21 1.21a16.5 16.5 0 006.364 6.364l1.21-1.21a2.25 2.25 0 012.157-.563l2.078.519A2.25 2.25 0 0119.5 19.71v2.04c0 1.243-1.007 2.25-2.25 2.25H16.5C9.044 24 3 17.956 3 10.5V9c0-1.243 1.007-2.25 2.25-2.25H6.75z"/></svg>
            </span>
            <div className="min-w-0">
              <div className="text-[11px] uppercase opacity-70">Phone</div>
              <div className="font-semibold text-xl sm:text-xl lg:text-lg break-words">+1 267-344-8878</div>
            </div>
          </div>
          <div className="flex items-center gap-4 sm:gap-6 lg:gap-4 py-4 sm:py-6 lg:py-4">
            <span className="w-6 h-6 sm:w-8 sm:h-8 lg:w-6 lg:h-6 grid place-items-center text-primary flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 sm:w-10 sm:h-10 lg:w-8 lg:h-8"><path d="M1.5 6.75A2.25 2.25 0 013.75 4.5h16.5A2.25 2.25 0 0122.5 6.75v10.5A2.25 2.25 0 0120.25 19.5H3.75A2.25 2.25 0 011.5 17.25V6.75zm3.03.72a.75.75 0 00-1.06 1.06l7.5 7.5a.75.75 0 001.06 0l7.5-7.5a.75.75 0 00-1.06-1.06L12 14.19 4.53 7.47z"/></svg>
            </span>
            <div className="min-w-0">
              <div className="text-[11px] uppercase opacity-70">Email</div>
              <div className="font-semibold text-xl sm:text-xl lg:text-lg break-words">info@itecexperts.com</div>
            </div>
          </div>
          <div className="flex items-center gap-4 sm:gap-6 lg:gap-4 py-4 sm:py-6 lg:py-4">
            <span className="w-6 h-6 sm:w-8 sm:h-8 lg:w-6 lg:h-6 grid place-items-center text-primary flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 sm:w-10 sm:h-10 lg:w-8 lg:h-8"><path fill-rule="evenodd" d="M12 2.25a9.75 9.75 0 100 19.5 9.75 9.75 0 000-19.5zM12.75 6a.75.75 0 00-1.5 0v6a.75.75 0 00.33.62l3.75 2.5a.75.75 0 10.84-1.24l-3.42-2.28V6z" clip-rule="evenodd"/></svg>
            </span>
            <div className="min-w-0">
              <div className="text-[11px] uppercase opacity-70">Office Hours</div>
              <div className="font-semibold text-xl sm:text-xl lg:text-lg break-words">9:00 AM - 6:00 PM</div>
            </div>
          </div>
        </div>

        <div className="border-b border-slate-200 py-3 mb-6 text-slate-700">
          <h4 className="text-3xl sm:text-3xl lg:text-2xl font-bold opacity-90 mb-6">Connect With Us</h4>
          <div className="flex gap-3 sm:gap-4">
            <button className="w-10 h-10 sm:w-14 sm:h-14 rounded text-primary grid place-items-center" aria-label="LinkedIn">
            ...
            <button className="w-10 h-10 sm:w-14 sm:h-14 rounded text-primary grid place-items-center" aria-label="Facebook">
            ...
            <button className="w-10 h-10 sm:w-14 sm:h-14 rounded text-primary grid place-items-center" aria-label="Instagram">
